...
import in.nkn.dao.ITouristRepo;
import in.nkn.model.Tourist;
import in.nkn.service.ITourisMgmtService;
...
public class TouristController {
	...
	@Autowired
	private ITourisMgmtService service;
	
	@PostMapping("/register")
	public ResponseEntity<String> enrollTourist(@RequestBody Tourist tourist) {
	    try {
	    	
	        String resultMsg = service.registerTouris(tourist);
	        return new ResponseEntity<String>(resultMsg, HttpStatus.OK);
	    } catch (Exception e) {
	        return new ResponseEntity<String>("Problem in tourist enrollment", HttpStatus.INTERNAL_SERVER_ERROR);// 500 E
	    }
